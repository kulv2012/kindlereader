...
<dc-metadata xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:opf="http://www.idpf.org/2007/opf">
    ...
    {% else %}
    <dc:title>{{ title }} ({{ mobitime.strftime("%m/%d %H:%M") }})</dc:title>
    {% end %}
...
class KindleReader(object):
    ...
    def main_processonefeed( self, feed ):
        ...
        if feed[0:4] == 'full':
            ...
            logging.info("asdf[{}/{}]:{}".format(feed_idx, feed_num, feed))
            feedq.put({'feed':feed, 'feed_idx':1})
        ...
        logging.info("asfasdf");
        if len(updated_feeds) > 0:
            title = updated_feeds[0]['title']
            mobi_file = self.make_mobi(krconfig.user, updated_feeds, krconfig.kindle_format, title)
